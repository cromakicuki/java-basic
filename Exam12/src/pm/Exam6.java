...
public class Exam6 extends Thread{ // Thread클래스를 상속받아서 클래스를 정의했고
	/*
	쓰레드 생성 방법
...
/*
 이전에 단일 쓰레드로 실행했던 비디오 프레임과 자막을 출력하는 프로그램을 멀티 쓰레드로 변경했다.
 출력하고자 하는 작업은 비디오 프레임의 번호와 자막의 동시 출력이다.
 프로그램이 처음 실행될 때 이미 main 쓰레드가 실행되고 있으므로 이 2개의 작업을 동시에 실행하기 위해서는 
 적어도 하나 이상의 쓰레드를 추가로 생성해서 실행해야 한다.
 
 Thread클래스를 상속받아서 클래스를 정의했고
 객체를 생성하여 start()메서드로 실행하였다.
 main 쓰레드와 smiFileThread를 사용하여 각각 비디오 번호와 자막번호를 출력했다.
 참고로 멀티 쓰레드는 독립적으로 실행되기 때문에 먼저 start()매서드로 호출이 되더라도
